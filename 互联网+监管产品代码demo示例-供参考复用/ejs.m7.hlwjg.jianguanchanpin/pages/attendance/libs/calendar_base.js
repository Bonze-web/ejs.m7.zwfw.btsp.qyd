...
(function () {
    ...
    Calendar.prototype = {
        ...
        _render: function (dataObj, callback) {
            ...
            self.options.dataRequest(dateStr, function (data) {
                ...
                for (var i = 0, len = dataObj.tmpInfo.length; i < len; i++) {
                    html += Mustache.render(self.options.dayTempl(dataObj.tmpInfo[i], self.currentDate), dataObj.tmpInfo[i]);
                }
        ...
        _addEvent: function () {
            ...
            // 给日期增加点击监听事件
            mui('.swiper-container').off();
            mui('.swiper-container').on('tap', '.em-calendar-item', function () {
                var dateStr = this.getAttribute('date');

                if (this.classList.contains('tipprev')) {
                    self.slidePrev();
                    Zepto('.swiper-slide-active [date="' + dateStr + '"]').addClass('em-calendar-active').siblings().removeClass('em-calendar-active');
                } else if (this.classList.contains('tipnext')) {
                    self.slideNext();
                    Zepto('.swiper-slide-active [date="' + dateStr + '"]').addClass('em-calendar-active').siblings().removeClass('em-calendar-active');
                } else {
