...
var common = window.common || (function(exports, undefined) {
	...
	exports.ajax = function(paramsData, successCallback, errorCallback, options) {
		...
		// 开启调试
		if(options.isDebug) {
			console.log("RESTful参数：\n" + Config.serverOA9Url + paramsData.url + "\n" + JSON.stringify({
				params: paramsData.data
			}) + "\n");
		}
		...
		var ajaxBody = {
		    // 兼容个性化接口地址传入
		    url: /http/.test(paramsData.url) ? paramsData.url : Config.serverOA9Url + paramsData.url,
			...
			success: function(result) {
				...
				// 执行外部回调
				(typeof(successCallback) == "function" && successCallback) ? successCallback(result): "that is not function!"
			},
