	...
	Calendar.prototype = {
		...
		refresh: function(year, month, day) {
			...
			if(self.options.isDebug) {
				console.log("日历数据格式：" + JSON.stringify(tmpInfo));
			}
			this._render(fileInfo, tmpInfo);
		},

		/**
		 * 视图的渲染和数据分离，采用内部的数据
		 */
		_render: function(fileInfo, tmpInfo) {
			var self = this;

			var html = '';
			html += '<div class="em-week">';
			html += '<span class="em-red">日</span>';
			html += '<span>一</span>';
			html += '<span>二</span>';
			html += '<span>三</span>';
			html += '<span>四</span>';
			html += '<span>五</span>';
			html += '<span class="em-red">六</span>';
			html += '</div>';

			html += '<div class="em-calendar-content">';
			for(var i = 0, len = tmpInfo.length; i < len; i++) {

				if(tmpInfo[i].date == self.currentDate) {
					if(tmpInfo[i].isforbid == 1) {
						html += '<div class="em-calendar-item em-calendar-active  isforbid' + tmpInfo[i].isforbid + '" date="' + tmpInfo[i].date + '">';
					} else {
						html += '<div class="em-calendar-item isforbid' + tmpInfo[i].isforbid + '" date="' + tmpInfo[i].date + '">';
					}
				} else {
					if(i == 15) {
						html += '<div class="em-calendar-item em-calendar-late isforbid' + tmpInfo[i].isforbid + '" date="' + tmpInfo[i].date + '">';
				...
				html += '<span class="day">' + tmpInfo[i].day + '</span>';
				html += '<p class="lunar">' + tmpInfo[i].lunar + '</p>';
				if(i == 16) {
					html += '<span class="dot dot-type1"></span>';
			...
			if(self.options.isDebug) {
				console.log("日历模板字符串：" + html);
			}

			this._selector(this.options.container).innerHTML = html;
		...
		_addEvent: function() {
			...
			// 给日期增加点击监听事件
			mui(self.options.container).on('tap', '.em-calendar-item', function() {
