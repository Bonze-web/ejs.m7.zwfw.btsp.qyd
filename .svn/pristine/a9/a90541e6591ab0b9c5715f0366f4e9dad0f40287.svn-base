...
(function (exports) {
     ...
     * 时间戳
     */
    exports.TIME_STAMP = '_=' + (exports.isDebug ? '20200313' : '2020313');
    ...
    /**
     * 部署态postcss自适应方案压缩css
     * 1为开启，0为关闭
     * 修改此配置后需要重新执行npm run build命令即可使部署态生效
     */
    exports.isUsePostCSS = 1;

    /**
     * 部署态是否将ES6语法编译成成ES5（支持async等语法）
     * 1为开启，0为关闭
     * 修改此配置后需要重新执行npm run build命令即可使部署态生效
     * 开启后对编译速度和.min文件大小有一定影响
     */
    exports.ES6BabelES5 = 0;
    ...
    /**
     * 开发环境常量，分别为：h5（wechat）、ejs、dingtalk、f9
     */
    var ENV_H5 = 'h5',
        // ejs 环境
        ENV_EJS = 'ejs',
...
(function (exports) {
    ...
    var SrcBoot = {
        ...
        getProjectBasePath: function (reg) {
            ...
            if (contextPath === '' && /index\.html/.test(patehName)) {
                basePath = obj.protocol + '//' + obj.host + (contextPath ? contextPath : '') + patehName.replace(/index\.html/, '');
            } else {
                // 暂时放一个兼容列表，兼容一些固定的目录获取
                var pathCompatibles = ['/showcase/', '/showcase_pending/', '/test/', '/'];
        ...
        changePathByConfig: function (path) {
            ...
            var lastPathRe = lastPath.replace('.' + suffix, '');

            lastPathRe += '.min.' + suffix;
            path = path.replace(lastPath, lastPathRe);
...
if (this.Config.ejsVer > 3) {
    paths.push(bizRootPath + 'js/libs/vant/index.css');
}
