...
(function() {
	...
	Calendar.prototype = {
		...
		_render: function(fileInfo, tmpInfo, activeSlideNode) {
			var self = this;
			self.getMySduleListTag(function(data) {
		 ...
		 * 获取我的日程业务列表
		 */
		getMySduleListTag: function(callback) {
			var self = this;
			...
			var data = {
				...
				"keyword": "",
				// 开始日期时间
				"fromdatetime": dateStr + "01 00:00:00",
				// 结束日期时间
				"todatetime": dateStr + "31 23:59:59",
			};
			//ejs.ui.showDebugDialog(JSON.stringify(data));
			//console.log(JSON.stringify(data));
			common.ajax({
				url: "schedule_getlist_v7",
				data: data,
				dataPath: "custom.schedulelist",
				isShowWaiting: false
			}, function(result) {

				var tmpInfo = result.data;
				// 日历
				var tmpCalendarData = [];
				if(tmpInfo && Array.isArray(tmpInfo) && tmpInfo.length > 0) {
					...
					mui.each(tmpInfo, function(key, value) {
						// 时间段
						value.time = DateUtil.getTime(value.fromdatetime, "hh:mm") + "-" + DateUtil.getTime(value.todatetime, "hh:mm");
						tmpCalendarData.push({
							date: DateUtil.getDate(value.fromdatetime, "yyyy-mm-dd")
						});
						tmpCalendarData.push({
							date: DateUtil.getDate(value.todatetime, "yyyy-mm-dd")
						});
					});

					// 去除数组重复项
					var resData = self.uniqueArray(tmpCalendarData, "date");
					//console.log("原数组：" + JSON.stringify(resData));
					//console.log("去除重复项后：" + JSON.stringify(resData));
			...
			var swiper = new Swiper('.swiper-container', {
				...
				onSlideNextStart: function(swiper) {
					...
					if(activeIndex == 2) {
						...
						self.refresh(self.nowYear, self.nowMonth, self.nowDay, document.querySelector(".swiper-slide").querySelector(".em-calendar-wrapper"));
					}
				},
			...
			if(self._selector(self.options.backToToday)) {
				self._selector(self.options.backToToday).addEventListener("tap", function() {
					...
					if(self.tmpActiveIndex == "1") {
						...
						self.refresh(y, m, d, swiper.slides[0]);
					} else {
						swiper.slideTo(1, 1000, false);
						// 非当前月时，切换，使用swiper.slides[1]
						self.refresh(y, m, d, swiper.slides[1]);
					}
