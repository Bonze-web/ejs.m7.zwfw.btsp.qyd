...
; (function (d, Util, $) {
  ...
  function initPage() {
    ...
    ejs.storage.getItem({
      key: 'adminMsg', // 或者 ['key1', 'akey2']
      success: function (result) {
        adminMsg = JSON.parse(result.adminMsg);
        // adminMsg = {
        ...
        actype = adminMsg.administrativecptype
        objname = adminMsg.administrativecptypevalue
        acguid = adminMsg.acguig
        objtypeText = adminMsg.administrativecp
        $('#personclass').val(objname)
        $('#person').val(objtypeText)
  ...
  function initListeners() {
    $('body')
      ...
      .on('tap', '#person', function (e) {
        ...
        ejs.page.open({
          pageUrl: '../temporary/choose_obj.html',
          pageStyle: 1,
          orientation: 1,
          data: {
            objecttype: actype,
      ...
      .on('tap', '#btn-thing', function (e) {
        // 获取监管事项
        ejs.page.open({
          pageUrl: '../temporary/multiple_selection_thing.html',
          pageStyle: 1,
          orientation: 1,
          data: {
            jgobjlist: JSON.stringify(jgobjlist),
          },
          success: function (result) {
      ...
      .on('tap', '#btn-law', function (e) {
        // 获取执法人员
        ejs.page.open({
          pageUrl: '../temporary/multiple_selection_law.html',
          pageStyle: 1,
          orientation: 1,
          data: {
            lawobjlist: JSON.stringify(lawobjlist),
          },
          success: function (result) {
  ...
  Zepto('#btn-addaction').on('tap', function () {
    ...
    ejs.page.open({
      pageUrl: '../temporary/temporary_supervision_add.html',
      pageStyle: 1,
      orientation: 1,
      data: {
  ...
  Zepto('#supervisionList').on('tap', '.edit-action', function (e) {
    ...
    ejs.page.open({
      pageUrl: '../temporary/temporary_supervision_detail.html',
      pageStyle: 1,
      orientation: 1,
      data: {
