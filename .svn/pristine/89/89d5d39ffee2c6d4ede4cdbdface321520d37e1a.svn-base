...
var customBiz = {
    ...
    initListeners: function () {
		...
		Zepto('.em-tab-list').on('tap','li',function(){
            var _this = this;
            self.administrativetype = Zepto(_this).attr('type');
...
//	代码项
	getCode: function () {
        ...
        function (result) {
            if (result.status.code == "1") {
                ...
                self.administrativetype = tmp[0].itemvalue;
                mui.each(tmp, function(key, value) {
                	html += Mustache.render(tabtemplate, value);
                })
                document.querySelector('.em-tab-list').innerHTML = html;
    ...
    initPullRefreshList: function () {
        ...
        var urlFunc = function () {
            var url = Config.serverUrl + 'checkInfo/getCheckItemList';
        ...
        var dataRequestFunc = function (currPage) {
            ...
            var data = {
                ...
                'pagesize': pageSize,
                // 处理类型
                'administrativetype': self.administrativetype,
                // 查询条件
        ...
        var itemClickFunc = function (e) {
        	var catalogid = Zepto(this).attr('catalogid');
        	var rowguid = Zepto(this).attr('rowguid');
        	var superviseitemcode = Zepto(this).attr('superviseitemcode');
        	ejs.page.open("./regulatory_detail.html", {
			    catalogid: catalogid,
			    rowguid: rowguid,
			    superviseitemcode: superviseitemcode
			});
        };
