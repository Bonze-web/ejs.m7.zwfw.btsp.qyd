...
(function(CommonTools) {
    ...
    function PullToRefresh(options) {
        ...
        // 生成下拉刷新对象,有一个默认值
        var PullToRefreshBase = null;

        switch (options.skin) {
            case 'type1':
                PullToRefreshBase = PullToRefreshTools.skin.type1;
                break;

            case 'type2':
                PullToRefreshBase = PullToRefreshTools.skin.type2;
                break;

            case 'type3':
                PullToRefreshBase = PullToRefreshTools.skin.type3;
                break;

            case 'type4':
                PullToRefreshBase = PullToRefreshTools.skin.type4;
                break;

            case 'type5':
                PullToRefreshBase = PullToRefreshTools.skin.type5;
                break;

            case 'native':
                PullToRefreshBase = PullToRefreshTools.skin.native;
                break;

            default:
                PullToRefreshBase = PullToRefreshTools.skin.defaults;
                break;
        }
    ...
    PullToRefresh.prototype = {
        ...
        _successRequest: function(response) {
            ...
            this.options.ajaxResponse = response
            if (!response) {
                if (options.isDebug) {
            ...
            if (options.success && typeof(options.success) === "function") {
                ...
                options.success(response, this.isPullDown || (this.currPage == options.initPageIndex), this.options.ajaxResponse);
            }
        ...
        _refreshState: function(isSuccess, dataLen) {
            ...
            instance.setSuccessTips && instance.setSuccessTips('刷新成功');
