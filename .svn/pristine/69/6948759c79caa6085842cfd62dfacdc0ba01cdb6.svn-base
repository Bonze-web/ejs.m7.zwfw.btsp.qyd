    ...
    function initListeners() {
        $('body')
            ...
            .on('tap', '.check-result-choose div', function (e) {
                ...
                if (parent[0].id == 'companyType') {
                    if (this.dataset.value == 'pdt') {
                        listname = '食品生产企业'
                        $('#nameSet label').text('生产者名称');
                        $('#nameSet input')[0].id = 'producter';
                        $('#tuanSet input')[0].id = 'organization';
                        $('.jystyle').addClass('hidden');
                    } else if (this.dataset.value == 'opt') {
                        listname = '食品经营企业'
                        $('#nameSet label').text('经营者名称');
                        $('#nameSet input')[0].id = 'operator';
                        $('#tuanSet input')[0].id = 'regiment';
                        $('.jystyle').removeClass('hidden');
                    }
                }
            })
            // 点击主体业态
            .on('tap', '#obj_state', function (e) {
            ...
            .on('tap', '#search', function (e) {
                ...
                arr.push({
                    text: '状态',
                    field: 'status',
                    value: $('#choose .active')[0].dataset.value || ''
                })
                console.log(arr)

                localStorage.setItem('inspection_searchresult', JSON.stringify(arr));
                ejs.page.open('./inspection_searchresult.html', {
                    listname: encodeURI(listname),
                    companyType: $('#companyType .active')[0].dataset.value
                });
            ...
            .on('tap', '#reset', function (e) {
                ...
                $('.check-result-choose').find('div').eq(0).addClass('active')
            })
    }
    /**
 * @description 获取代码项
