...
var customBiz = {
	...
	initListeners: function() {
		var self = this;
		// 默认类型
		self.infotype = Util.getExtraDataByKey("infotype") || '2';
		if(self.infotype == '7') {
			Zepto('.em-header-scroll-item').eq(1).addClass('em-active').siblings().removeClass('em-active');
		}
		...
		// 头部切换
		mui('.em-header-scroll').on('tap', '.em-header-scroll-item', function() {
			var _this = this;
			self.infotype = Zepto(_this).attr('infotype');
	...
	initPullRefreshList: function() {
		...
		var urlFunc = function() {
			var url = Config.serverUrl + 'centerInfo/getInfoList';
			console.log(url);
		...
		var dataChangeFunc = function(response) {
			...
			if(response.status.code == '1') {
				outData = response.custom.infolist || [];
			}
		...
		var itemClickFunc = function(e) {
			var infoid = Zepto(this).attr('infoid');
			ejs.page.open("./notice_detail.html", {
				infoid: infoid,
			});
		};
