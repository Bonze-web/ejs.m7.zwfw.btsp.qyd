...
var customBiz = {
  ...
  initListeners: function () {
    ...
    self.itemguid = Util.getExtraDataByKey('itemguid') || ''
    ...
    //下载
    mui('.mui-content').on('tap', '.em-todo-attach-pic', function () {
      var _this = this
      var attachpath = Zepto(_this).parents().attr('attachpath')
      var attachfilename = Zepto(_this)
        .siblings()
        .children()
        .children('.em-todo-attach-name')
        .text()
  ...
  initPullRefreshList: function () {
    ...
    var dataChangeFunc = function (response) {
      ...
      if (response.status.code == '1') {
        outData = response.custom.behaviorlist || []
      }
      mui.each(outData, function (key, value) {
        //获取附件图标、大小
        if (value.attachlist.length > 0) {
          mui.each(value.attachlist, function (key, value) {
            value.attachlength = FormatFile.getKMByByte(value.attachlength)
            value.attachfileicon = FormatFile.getFileIcon(value.attachfilename)
          })
        }
      })
