...
var customBiz = {
  ...
  initListeners: function () {
    ...
    // 头部切换
    Zepto('#head').on('tap', '.headitem', function () {
      var _this = this
      self.plantype = Zepto(_this).attr('type')
      if (self.plantype == '30') {
        Zepto('#second').addClass('mui-hidden')
        Zepto('#minirefresh').css('marginTop', '40px')
      } else {
        Zepto('#second').removeClass('mui-hidden')
        Zepto('#minirefresh').css('marginTop', '77px')
      }
      Zepto(_this).addClass('headactive').siblings().removeClass('headactive')
      self.PullToRefreshToolObj.refresh()
    })

    // 二级tab切换
    Zepto('#second').on('tap', '.seconditem', function (e) {
      var _this = this
      self.sectype = Zepto(_this).attr('sectype')
      Zepto(_this)
        .addClass('secondactive')
        .siblings()
        .removeClass('secondactive')
      self.PullToRefreshToolObj.refresh()
    })
  },
  // 刷新列表
  initPullRefreshList: function () {
    ...
    var itemClickFunc = function (e) {
      if (self.plantype != '30') {
        var parentguid = Zepto(this).attr('rowguid')
        ejs.page.open('./random_list.html', {
          plantype: self.plantype,
    ...
    self.PullToRefreshToolObj = new MiniRefreshBiz({
      ...
      theme: ejs.os.ejs
        ? MiniRefreshTools.theme.natives
        : MiniRefreshTools.theme.defaults,
      isDebug: true,
