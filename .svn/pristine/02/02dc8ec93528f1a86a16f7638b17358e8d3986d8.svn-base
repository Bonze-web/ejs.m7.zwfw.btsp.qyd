...
;(function (d, $) {
  ...
  function initPage() {
    ...
    dutyguid = Util.getExtraDataByKey('rowguid') || ''
  ...
  function appendFileWrappper(files, fileObj, attachguid) {
    ...
    console.log('.attach-view' + curridx)
  ...
  function deleteAttach(attachindex, _parentNode, _this) {
    ...
    console.log(_parentNode.children)
    dealAttach(attachindex, _parentNode)
  }
   ...
   * 附件删除
   */
  function dealAttach(guid, parent) {
    ...
    Util.ajax({
      ...
      success: function (result) {
        ...
        console.log('.em-attach-count' + curridx)
        ejs.ui.toast('删除成功')
        console.log(document.querySelector('.em-attach-count' + curridx))
        // 刷新附件个数
        document.querySelector('.em-attach-count' + curridx).innerText =
          parent.children.length
        if (parent.children.length == 0) {
          // 隐藏附件
          document.querySelector('.em-attach-wrap' + curridx).style.height =
            '0px'
        }
        // getSuperviseInfo();
      },
