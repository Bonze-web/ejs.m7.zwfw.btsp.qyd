 ...
 * 版本: [1.0, 2017/07/12 ]
 * 版权: 江苏国泰新点软件有限公司
 * 描述: sha256散列操作相关，比 sha1更安全
 * 散列算法更多的用来进行数据签名，而不是加密，因为虽然不可逆，但是可以通过彩虹表轻易撞库破解常见密码
 * 但可以通过加盐增加安全性，加盐过程如下
 * 客户端请求服务端
 * 服务端随机生成一个盐值(比如uuid生成方式)，存入数据库，并且将盐值传给客户端
 * 客户端将sha1生成后的密码 + 盐值再次sha1散列(规则要定好，比如  盐值+sha1密码)
 * 服务端接收到密码后，将数据库中存储的sha1密码与盐值按照规则组合再次sha1，然后进行对比，如果匹配则正确
 * 注意，盐值是随机生成的，并且是短期存在的，新的请求会有全新的随机盐值，这样才能避免被抓包，泄漏密码
 */
(function() {
    'use strict';
