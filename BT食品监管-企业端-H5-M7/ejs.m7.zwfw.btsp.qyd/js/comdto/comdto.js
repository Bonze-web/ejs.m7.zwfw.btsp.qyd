...
(function (exports, controlMap) {
    ...
    extend(WebUploader, MControl, {
        ...
        _init: function () {
            ...
            this.input.addEventListener('change', function () {
                ...
                if (filelist.firstElementChild && filelist.firstElementChild.textContent === '暂未上传文件') {
                    filelist.innerHTML = '';
                }
            ...
            mui(this.el).on('tap', '[data-role="preview"]', function () {
                var el = this;

                while (!(el.dataset.type === 'body')) {
                    el = el.parentElement;
                }
                ...
                if (filetype.indexOf('image') !== -1) {
                    ejs.util.prevImage({
                        index: 0,
                        selectedPhotos: [epm.dealUrl(downloadurl)],
                        success: function () {
                        }
                    });
                } else {
                    ejs.util.getPreviewUrl({
                        success: function (result) {
                            ...
                            if (previewUrl.indexOf('?furl') === -1) {
                                Util.ejs.ui.toast('officeweb365地址配置错误，或未配置');
                                return;
                            }

                            ejs.page.open(previewUrl + epm.dealUrl(downloadurl));
                        }
                    });
                }
            });
        ...
        _upload: function (formdata) {
            ...
            Util.ajax({
                ...
                success: function (response) {
                    ...
                    // var data = epm.getSecondRequestData(response).data;
                    // console.log('data');
                    // console.log(data);
        ...
        setData: function (data) {
            ...
            var item = '';
            var filelist = this.el.querySelector('.webuploader-filelist');
            var tpl = this.tpl;
    ...
    extend(SelectContact, MControl, {
        ...
        _init: function () {
            ...
            this.bind = this.getAttribute('bind');
            this.action = this.getAttribute('action');
            this.isouonly = this.getAttribute('isouonly') || false; // 是否仅选择部门
    ...
    extend(HandleControls, MControl, {
        ...
        _setRightBtn: function (isShow) {
            ...
            ejs.navigator.setRightBtn({
                ...
                success: function () {
                    ...
                    console.log("*submitBefore***********333" + submitBefore);
        ...
        _HeaderSubmit: function () {
            var aipNode = "";
            try {
                if (typeof (eval(sxqpsubmit)) == "function") {
                    aipNode = window.sxqpsubmit();
                }
            } catch (e) {
            }
            ...
            epointm.execute('getPageUrlOfOperate', '@none', epm.encodeUtf8(epm.encodeJson(params)), function (response) {
                var _AjaxOperationHd = self._AjaxOperationHd.bind(self);
                // Util.ejs.ui.alert("aipNode************" + aipNode);
                _AjaxOperationHd(response, aipNode);
            });
        },
        // 通用的ajax方法返回结果处理
        _AjaxOperationHd: function (response, aipNode) {
            var self = this;
            if (response) {
                        ...
                        success: function (result) {
                            if (result.which == 1) {
                                epointm.execute("getoperate", "@none", epm.encodeUtf8(epm
                                    .encodeJson(response)), function (response) {
                                        var _AjaxOperationHd = self._AjaxOperationHd.bind(self);
                                        _AjaxOperationHd(response, "");
                                    });
                    ...
                    ejs.page.open({
                        ...
                        success: function (result) {
                            if (result && result.resultData.message) {
                                console.log(result.resultData.message);
                                result.resultData.message.aipNode = aipNode;
                                console.log(result.resultData.message);
        ...
        var epointm = {
            ...
            validate: function (controls) {
                ...
                for (var key in controls) {
                    ...
                    if (el.required) {
                        if (value === '') {
                            var text = ($(el).parent().attr('label') || $(el).parent().text() || $(el).parent().prev().text()).replace(/[:: ]$/, '');
                            Util.ejs.ui.toast(el.getAttribute('requiredErrorText') || (text + '不能为空'));
                            return false;
                        }
                    }
