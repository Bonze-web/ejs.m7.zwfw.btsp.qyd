...
function getFullPath(path) {
    // 全路径
    if (/^(http|https|ftp|epth|h5|\/\/)/g.test(path)) {
        return path;
    }
    ...
    function callApi(options) {
        ...
        var callback = function callback(resolve, reject) {
            callJsBridge({
                ...
                proto: finalOptions.mudule || finalOptions.module,
...
function appMixin(hybrid) {
    ...
    var detect = function detect() {
        this.app = function (opt) {
            if (hybridJs.os.ejs) {
                hybridJs.event.registerEvent({
                    key: 'created',
                    success: function success() {
                        opt.onLaunch && opt.onLaunch();
                    }
                });
                hybridJs.event.registerEvent({
                    key: 'resume',
