...
(function (globalContext, factory) {
    ...
    var innerUtil = window.MiniRefreshTools,
        ...
        ajaxRequest = Util.ajax,
        // 采用统一的数据处理方式，获得统一数据
        dataProcess = Util.dataProcess;
    ...
    MiniRefreshBizlogic.prototype = {
        ...
        _pullUpCallback: function () {
            ...
            if (!this.isrefresh && !this.loadingUp) {
            ...
            if (options.isAutoRender) {
                ...
                if (renderTemplateEngine) {
                    if (response && Array.isArray(response) && response.length > 0) {
                        ...
                        if (outList !== '') {
                            this.listContainer.innerHTML += outList;
                        }
                    } else {
                        // 没有返回数据,代表不可以加载更多
        ...
        _ajaxRequest: function () {
            ...
            if (this.currPage > options.initPageIndex) {
                options.up = true;
            } else {
                options.up = false;
            }
        ...
        _successRequest: function (response) {
            ...
            this._refreshState(dataLen > 0 ? true : false, dataLen);
        },
        ...
        refreshNosense: function () {
            var currPage = this.currPage,
                options = this.options,
                initPageIndex = options.initPageIndex,
                dataChange = options.dataChange,
                optionsTemplate = options.template;
            ...
            if (options.dataRequest) {
                for (; initPageIndex <= currPage; initPageIndex++) {
                    var requestData = options.dataRequest(initPageIndex, function (requestData) {
                        promiseArr.push(next(requestData));
                    });
            ...
            Promise.all(promiseArr).then(function (data) {
                if (data && Array.isArray(data)) {
                    data.forEach(function (e) {
                        ...
                        if (dataChange) {
                            // 如果存在转换数据的函数，用外部提供的
                            response = dataChange(e);
                        } else {
                            // 使用默认的数据转换
                            response = self._dataChangeDefault(e);
                        }
                        ...
                        if (optionsTemplate) {
                            if (typeof optionsTemplate === 'string') {
                                ...
                                // 如果是模版字符串
                                template = optionsTemplate(response);
                            }
                        }
                        ...
                        if (renderTemplateEngine) {
                            if (response && Array.isArray(response) && response.length > 0) {
                                for (var i = 0, len = response.length; i < len; i++) {
                                    outList += renderTemplateEngine.render(template, response[i]);
                                }
                            }
                        }
                    });
                ...
                self.listContainer.innerHTML = outList;
            });
        },
        ...
        _ajaxRequestNosense: function () {
            ...
            var dataChange = options.dataChange;
                ...
                ajaxRequest({
                    ...
                    success: function (response) {
                        if (dataChange) {
                            ...
                            // 使用默认的数据转换
                            response = self._dataChangeDefault(response);
                        }
