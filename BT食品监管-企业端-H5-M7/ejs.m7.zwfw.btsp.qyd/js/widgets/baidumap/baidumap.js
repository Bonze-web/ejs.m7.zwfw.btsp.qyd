 ...
 * 需要依赖于百度地图sdk-forjs
 * 2020/12/18 EJS内使用百度地图组件API wsze
 */
/* global BMap:true */
(function () {
    'use strict';
    ...
    var BMAP_STATUS_SUCCESS = window.BMAP_STATUS_SUCCESS || 0;
    ...
    BaiduMap.prototype = {
         ...
         * @param {Boolean} isOrigin 是否是原始坐标，原始坐标和谷歌坐标有些偏差
         */
         ...
         */
        _getLocationByEJS: function (success, error) {
            // 自定义api获取经纬度;
            this._callApiGetLocation(success);
            ...
            //     },
            //     error: function (err) {
            //         error && error(err.msg || 'EJS获取坐标失败');
            //     },
            // });
        },
        /**
         * 通过自定义api获取定位（ 满足条件：需要原生配置文件，h5打包选择百度组件，对应js需要注册组件）
         * @param {Function} success 成功回调
         * @param {Function} error 失败回调
         */
        _callApiGetLocation: function (success) {
            window.JSBridge.callHandler(
                'util',
                'invokePluginApi', {
                    path: 'baidumap.provider.location',
                    dataMap: {},
                },
                function (data) {
                    if (data.code == '1') {
                        success && success(data.result.longitude, data.result.latitude, 0);
                    } else {
                        var msg0 = 'Log：' + data.msg + '\n\n';
                        var msg1 = '解决方案：\n1、请前往百度地图开发者平台申请appkey\n';
                        var msg2 = '2、确保前端EJS为3.2.4以上\n\n';
                        var msg3 = '3、最新版本组件需在 build.gradle或者 project.gradle 中加入,不然打包会报错\n\n';

                        ejs.ui.confirm({
                            title: '温馨提示',
                            message: msg0 + msg1 + msg2 + msg3,
                            buttonLabels: ['关闭当前页面'],
                            cancelable: 0,
                            success: function (result) {
                                ejs.page.close();
                ...
                function (error) {
                    ...
                    error && error(error.msg || '自定义api获取坐标失败');
                }
            );
        },
        /**
        ...
        getLocation: function (success, error) {
            ...
            } else if (options.isForceEJS && Util.os.ejs && window.ejs && ejs.runtime.getGeolocation) {
                this._getLocationByEJS(callback, error);
