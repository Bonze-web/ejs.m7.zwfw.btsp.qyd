 ...
 */
 ...
 (function() {
    ...
    // TODO(sunzl): 源码需要优化，不影响使用，后期完善优化
    /**
	 * 全局生效默认设置
	 * 默认设置可以最大程度的减小调用时的代码
	 */
    ...
    Calendar.prototype = {
        ...
        _render: function(fileInfo) {
            ...
            for (var j = 1; j < 7; j++) {
                html += '<tr>';
                var perTD = '<td><div class="dateQuan"><span class="day" data-value="{{day_index}}">{{day_index}}</span><span class="font12">{{lunar_index}}</span></div></td>';
                var output = '';
        ...
        setInfo: function (infolist) {
            ...
            // 渲染迟到早退等
            Zepto('.dateQuan').each(function () {
                var day = parseInt(Zepto(this).find('.day').data('value'));

                if(day < 10){
                    day = '0' + day;
                }

                let thisData = self.nowYear + '-' + self.nowMonth + '-' + day;
                
                if(!Zepto(this).hasClass('nouse') && infolist.includes(thisData)){
                    Zepto(this).addClass('chooseDate')
                }
        ...
        _addEvent: function() {
            ...
            mui(self.options.container).on('tap', '.dateQuan', function() {
                ...
                if (Zepto(this).hasClass('nouse')) {} else {
                    Zepto('.dateQuan').removeClass('clickActive');
                    Zepto(this).addClass('clickActive');
                    var day = this.querySelector(".day").getAttribute("data-value");
                    if (parseInt(day) > 9) {
                        day = '' + parseInt(day) + '';
                    } else {
                        day = '0' + parseInt(day) + '';
