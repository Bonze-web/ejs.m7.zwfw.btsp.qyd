...
(function () {
     ...
     * @param {Object} xhrError 失败信息
     */
    function notifyContaineRefreshToken(success, error, xhrError) {
        // 添加到回调中
        ...
        if (ejs.os.ejs && Config.ejsVer >= 3) {
            // 429状态进入，用于验证频繁请求
            if (xhrError) {
                pluginVersion('EJSFramework', '3.5.0').then(function () {
                    ...
                    if (error.xhr) {
                        ...
                        if (status === 417) {
                            pluginVersion('EJSFramework', '3.5.0').then(function () {
                                ejs.auth.logoutUserWithAlert({
                                    title: '提醒',
                                    message: '您的账号已在其他设备登录，若不是本人操作，请尽快修改密码！',
                                    success: function (result) {},
                                    error: function (error) {}
                                });
                            });
                            ...
                            pluginVersion('EJSFramework', '3.5.0').then(function () {
                                var xhrError = error.xhr.responseText;
